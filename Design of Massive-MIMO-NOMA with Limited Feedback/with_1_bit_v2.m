...
%%%%%%%%%%%%%%%%%%%%%% start the iteration
a1 = 3/4; a2 = 1/4;  R1=1.9; R2=4;   ct=100000; 
...
%       +  ( Fp(1/taux,ai, N, Mt))^2 -0.5*( Fp(snr*xi1,ai, N, Mt))^2 -0.5*( Fp(snr*xi2,ai, N, Mt))^2;
 ...
 pa1(k) = 2*(1 - Fp(max(1/taux, snr*xi1),ai, N, Mt))*Fp(1/taux,ai, N, Mt)+0.5*(1 - (Fp(max(1/taux, snr*xi1),ai, N, Mt))^2) + 0.5*(1 - (Fp(max(1/taux, snr*xi2),ai, N, Mt))^2) -  (1 - Fp(max(1/taux, snr*xi1),ai, N, Mt))*Fp(1/taux,ai, N, Mt)...
     - (1 - Fp(max(1/taux, snr*xi2),ai, N, Mt))*Fp(1/taux,ai, N, Mt) +    max(0,0.5*( Fp(1/taux,ai, N, Mt))^2 -0.5*( Fp(snr*xi1,ai, N, Mt))^2) + max(0,0.5* (Fp(1/taux,ai, N, Mt))^2 - 0.5*(Fp(snr*xi2,ai, N, Mt))^2);
  
end
 semilogy(snrdb, p1, snrdb,p2, snrdb, pa1,snrdb,p3, snrdb,p4,snrdb,pa2)
