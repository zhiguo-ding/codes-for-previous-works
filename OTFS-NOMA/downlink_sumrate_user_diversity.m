clear all 
 ...
 for ki =  1:length(snrdb)
  ...
  for ix = 1 :ct            
    ...
    %%%%%% FD-ZF
    SINRi1 = snr*gamma0/(snr*gamma1 + sum(1./ (di))/M);
    
    if SINRi1 >eps0 &snr*gamma1* (di(1))>epsi
        sum3 = sum3 +1;
    end
    
    %%%%%% FD-DFE
    SINRi2 = snr*gamma0/(snr*gamma1 +1/min((di)));
    
    if SINRi2 >eps0 & snr*gamma1* (di(1))>epsi
        sum4 = sum4 +1;
    end       
  end
 ...
 plot( snrdb,(1-p1)*R0+(1-p3)*Ri,snrdb,(1-p2)*R0+(1-p4)*Ri)
%  plot(snrdb, (1-p0)*R0,snrdb, (1-p01)*R0,...
%      snrdb,(1-p1)*R0+(1-p3)*Ri,snrdb,(1-p2)*R0+(1-p4)*Ri)